...
Download_Server_Status_client(){
	...
	if [[ ! -e "${client_file}" ]]; then
		mkdir "${client_file}"
		mv "/tmp/client-psutil.py" "${client_file}/client-psutil.py"
	else
		if [[ -e "${client_file}/client-psutil.py" ]]; then
			mv "${client_file}/client-psutil.py" "${client_file}/client-psutil1.py"
			mv "/tmp/client-psutil.py" "${client_file}/client-psutil.py"
		else
			mv "/tmp/client-psutil.py" "${client_file}/client-psutil.py"
		fi
	...
	if [[ ! -e "${client_file}/client-psutil.py" ]]; then
		...
		[[ -e "${client_file}/client-psutil1.py" ]] && rm -rf "${client_file}/client-psutil1.py"
		rm -rf "/tmp/client-psutil.py"
	fi
}
...
Install_caddy(){
	...
	if [[ "${caddy_yn}" == [Yy] ]]; then
		...
		if [[ ! -e "/usr/local/caddy/caddy" ]]; then
			wget -N --no-check-certificate https://raw.githubusercontent.com/lastle/doubi/master/caddy_install.sh
...
Update_Shell(){
	...
	wget -N --no-check-certificate "https://raw.githubusercontent.com/lastle/doubi/master/status_91yun.sh" && chmod +x status.sh
